            ...
            <div class="answer-comment order-response" style="margin-left:<?php echo ($response->level - 1)*20;?>px;">
                ...
                <div class="right-align">
                    <?php if($order->userId == Yii::app()->user->id && !$order->juristId):?>
                        <?php echo CHtml::link('Выбрать этого юриста исполнителем', Yii::app()->createUrl('/order/setJurist', ['id' => $order->id, 'juristId' => $response->authorId, 'price' => $response->price]), ['class' => 'btn btn-xs btn-success']);?>
                    <?php endif;?>
