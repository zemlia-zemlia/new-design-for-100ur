...
<p class="text-center">Наша партнерская программа работает по двум моделям<br/> CPL (Cost Per Lead − оплата за лид) и
    <br/>CPA (Cost Per Action – оплата за действие). </p>

<strong>
    <p>Мы предоставляем возможность рекламодателям увеличить количество клиентов, а вебмастерам выгодно монетизировать
        интернет трафик.</p>
</strong>
...
<p>
    Для владельцев источников трафика есть возможность его монетизации по следующему принципу: После регистрации вам
    будет доступна индивидуальная реферальная ссылка которую вы размещаете на своих сайтах, пабликах, группах и т.д. При
    переходе по вашей ссылке в нашу партнерскую программу передается информация о источнике конкретного
    пользователя.<br/> Начисление оплаты на ваш счет происходит за каждый опубликованный на сайте партнерской программы
    вопрос содержащий в себе вопрос юридического характера, оплачивается только один (первый) вопрос от каждого
    уникального пользователя. Сумма начисления фиксированная и не зависит от региона пользователя который задал свой
    вопрос.
    <blockquote>
        <h3>Сумма вознаграждения за каждый опубликованный вопрос
            <br/><strong> <?php echo MoneyFormat::rubles(Yii::app()->params['questionPrice']); ?> рублей</strong>
        </h3>
...
<p>В отличии от первого варианта партнерки в этом случае оплачиваются только активные в данный момент времени регионы.
    Активные регионы могут добавляться и удаляться.</p>

<strong>
    <p>Вы можете работать с нами по обоим вариантам партнерской программы одновременно и пользуясь всеми
        привилегиями.</p>
</strong>
