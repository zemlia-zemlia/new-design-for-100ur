...
class SiteController extends Controller
{
    ...
    public function actionContacts()
    {
        ...
        if (isset($_POST['ContactForm'])) {
            ...
            if ($contactForm->validate()) {                // Пытаемся отправить письмо
                ...
                $mailer->message = 'Имя отправителя: ' . CHtml::encode($contactForm->name) . '<br />' . 'Email отправителя: ' . CHtml::encode($contactForm->email) . '<br />' . 'Сообщение:<br />' . CHtml::encode($contactForm->message);
                if ($mailer->sendMail()) {
                    $formResult = ['code' => 0, 'message' => 'Сообщение успешно отправлено'];
                    $contactForm = new ContactForm();
                } else {
                    $formResult = ['code' => 500, 'message' => 'Ошибка, не удалось отправить сообщение'];
                }
            }
        }
    ...
    public function actionBrakLead($code)
    {
        ...
        if (isset($_POST['Lead'])) {
            ...
            if (Lead::BRAK_REASON_BAD_REGION == $lead->brakReason && !$lead->newTownId) {
                $lead->addError('brakReason', 'Не указан новый город');
            }
            if (Lead::BRAK_REASON_BAD_REGION == $lead->brakReason && $lead->newTownId == $lead->townId) {
