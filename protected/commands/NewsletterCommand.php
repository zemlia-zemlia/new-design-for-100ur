    ...
    public function actionIndex()
    {
        ...
        foreach($users as $user) {
            ...
            foreach($posts as $post) {
                ...
                $mailer->message .= "</td><td style='padding-left:10px;'><h2>" . CHtml::link(CHtml::encode($post->title), Yii::app()->createUrl('post/view', array('id'=>$post->id))) .
                        "</h2><p>" .
                        $post->preview .
                        CHtml::link("Читать на сайте", Yii::app()->createUrl('post/view', array('id'=>$post->id)))
                        . "</p></td></tr>";
            }
...
Вы получили это письмо, так как подписались на рассылку правовых инструкций на www.100yuristov.com. 
                Чтобы отписаться от рассылки, нажмите на <a href='" . Yii::app()->createUrl('user/unsubscribe', array('email'=>$mailer->email, 'code'=>md5(User::UNSUBSCRIBE_SALT.$mailer->email))). "'>ссылку</a>.
