...
class SendAnswerNotificationCommand extends CConsoleCommand
{
    ...
    public function actionIndex()
    {
        ...
        $emailsSent = []; // массив с адресами, на которые уже отправили письма

        $criteria = new CDbCriteria();
        $criteria->addCondition('DATE(t.`datetime`)>NOW()-INTERVAL 40 DAY');

        $answers = Answer::model()->with(['question', 'question.author'])->findAll($criteria);
        ...
        foreach ($answers as $answer) {
            ...
            if (true === $user->sendAnswerNotification($question, $answer)) {
