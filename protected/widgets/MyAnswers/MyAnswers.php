...
class MyAnswers extends CWidget
{
    ...
    public function run()
    {
        $answersTotalRow = Yii::app()->db->cache($this->cacheTime)->createCommand()
                ...
                ->from("{{answer}}")
                ->where("authorId=:authorId AND status IN(:statusNew, :statusPub)", array(
        ...
        $answersMonthRow = Yii::app()->db->cache($this->cacheTime)->createCommand()
                ->select("COUNT(*) counter")
                ->from("{{answer}}")
                ->where("authorId=:authorId AND status IN(:statusNew, :statusPub) AND datetime>NOW()-INTERVAL 30 DAY", array(
                        ':authorId'     =>  Yii::app()->user->id,
                        ':statusNew'    =>  Answer::STATUS_NEW,
                        ':statusPub'    =>  Answer::STATUS_PUBLISHED,
                    ))
        ...
        $answersDayRow = Yii::app()->db->cache($this->cacheTime)->createCommand()
                ->select("COUNT(*) counter")
                ->from("{{answer}}")
                ->where("authorId=:authorId AND status IN(:statusNew, :statusPub) AND DATE(datetime)=DATE(NOW())", array(
                        ':authorId'     =>  Yii::app()->user->id,
                        ':statusNew'    =>  Answer::STATUS_NEW,
                        ':statusPub'    =>  Answer::STATUS_PUBLISHED,
                    ))
