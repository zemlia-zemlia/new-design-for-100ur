...
?>
    ...
    echo CustomFuncs::niceDate($data->createDate, false, false);
                        ...
                        <?php
                        if ($data->publishDate) {
                            echo "<span class='muted'>Опубликован " . CustomFuncs::niceDate($data->publishDate) . ' ' . CHtml::link(CHtml::encode($data->bublishUser->name . ' ' . $data->bublishUser->lastName), Yii::app()->createUrl('question/byPublisher', ['id' => $data->bublishUser->id])) . '</span>';
                        }
                        ?>
                        &nbsp;
                    <?php endif; ?>
