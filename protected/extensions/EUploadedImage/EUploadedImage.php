...
class EUploadedImage extends CComponent
{
    ...
    public static function getInstanceByName($name)
    {
        ...
        return isset(self::$_files[$name]) && UPLOAD_ERR_NO_FILE != self::$_files[$name]->getError() ? self::$_files[$name] : null;
    }
    ...
    public static function getInstancesByName($name)
    {
        ...
        foreach (array_keys(self::$_files) as $key) {
            if (0 === strncmp($key, $name, $len) && UPLOAD_ERR_NO_FILE != self::$_files[$key]->getError()) {
    ...
    protected function __construct($name, $tempName, $type, $size, $error)
    {
        ...
        if (IMAGETYPE_JPEG === $image_info[2] || IMAGETYPE_PNG === $image_info[2] || IMAGETYPE_GIF === $image_info[2]) {
     ...
     *
     * @return bool true whether the image is saved successfully
     */
    ...
    {
        return UPLOAD_ERR_OK != $this->_error;
    }
